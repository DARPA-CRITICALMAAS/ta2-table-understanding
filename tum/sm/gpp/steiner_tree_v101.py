...
from tum.gpp.postprocessing.cgraph import CGEdge, CGEdgeTriple, CGNode, CGraph
from tum.gpp.postprocessing.common import add_context, ensure_valid_statements
from tum.gpp.postprocessing.config import PostprocessingConfig


class SteinerTreeV101:
    def __init__(
        self,
        table: FullTable,
        cg: CGraph,
    ...
    def compare_solutions(self, a: Solution, b: Solution) -> int:
        ...
        for s in [a, b]:
            if not hasattr(s, "meet_hard_constraints"):
                setattr(s, "meet_hard_constraints", self.meet_hard_constraints(s))

        if not getattr(a, "meet_hard_constraints") and getattr(
            b, "meet_hard_constraints"
        ):
            return 1
        if getattr(a, "meet_hard_constraints") and not getattr(
            b, "meet_hard_constraints"
        ):
            return -1
    ...
    def meet_hard_constraints(self, g: Solution) -> bool:
        # statements should not have multiple edges of same predicates
        for u in g.graph.iter_nodes():
            cgu = self.cg.get_node(u.id)
            if cgu.is_statement_node:
                # check if there are multiple edges of same predicates
                pid2count = defaultdict(int)
                outedges = g.graph.out_edges(u.id)
                for outedge in outedges:
                    pid2count[outedge.key] += 1
                    if pid2count[outedge.key] > 1:
                        # there is more than one edge of the same predicates -> false
                        return False

                # the qualifier should not have any outgoing edges.
                inedges = g.graph.in_edges(u.id)
                if len(inedges) > 0:
                    assert len(inedges) == 1
                    main_prop_id = inedges[0].key
                    for outedge in outedges:
                        if outedge.key != main_prop_id:
                            if g.graph.out_degree(outedge.target) > 0:
                                # the qualifier should not have any outgoing edges.
                                return False
